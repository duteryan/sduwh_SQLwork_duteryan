...
def get_user_orders(
        request: Request,
        status: str = None,
        sort: str = "date_desc"  # 排序方式：date_asc, date_desc, price_asc, price_desc
):
    token = request.headers.get("Authorization", "").replace("bearer ", "")
    conn = None
    try:
...
def get_sold_items(
    request: Request,
    status: str = None,  # 商品状态筛选：available/ordered/completed/cancelled
    sort: str = "date_desc"  # 排序方式：date_asc/date_desc/price_asc/price_desc
):
    token = request.headers.get("Authorization", "").replace("bearer ", "")
    conn = None
...
def get_all_items(request: Request):
    ...
    try:
        ...
        search_mode = request.query_params.get("mode", "")
        search_value = request.query_params.get("value", "")
        sort_mode = request.query_params.get("sort", "create_time")  # 默认时间排序
        ...
        base_sql += f" ORDER BY {sort_field} DESC"  # 降序排序（可根据需求改为ASC）
