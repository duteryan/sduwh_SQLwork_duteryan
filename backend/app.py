...
# 用户已收货订单
@app.get("/api/user/received-orders")
def get_received_orders(request: Request):
    token = request.headers.get("Authorization", "").replace("bearer ", "")
    conn = None
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        ...
        cursor.execute("""
            ...
            WHERE o.BuyerID = ? AND o.Status = 'completed'
...
async def submit_review(order_id: int, request: Request):
    token = request.headers.get("Authorization", "").replace("bearer ", "")
    conn = None
    try:
        ...
        cursor.execute("""
            ...
            FROM Orders o
            LEFT JOIN Review r ON o.OrderID = r.OrderID
            WHERE o.OrderID = ? AND o.BuyerID = ? AND o.Status = 'completed' AND r.ReviewID IS NULL
        """, (order_id, user_id))

        if not cursor.fetchone():
            return {"success": False, "message": "无法评价此订单（可能已评价或订单状态不正确）"}
...
async def get_review_detail(order_id: int, request: Request):
    token = request.headers.get("Authorization", "").replace("bearer ", "")
    conn = None
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        ...
        if not review:
            return {"success": False, "message": "此订单尚未评价"}
