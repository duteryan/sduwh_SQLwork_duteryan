    ...
    content.innerHTML = `
        <div class="my-panel">
            <div class="my-sidebar">
                ...
                <div class="my-tab" onclick="switchMyTab(this, 'reviews')">订单评价</div>
...
function switchMyTab(element, tabName) {
    ...
    else if (tabName === 'reviews') loadOrderReviews();
...
function initReviewModal() {
    const modalHtml = `
        <div class="modal" id="review-modal" style="display: none; z-index: 9999; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); align-items: center; justify-content: center;">
            <div class="card" style="width: 400px; padding: 20px; border-radius: 8px; background: white; position: relative;">
                <span class="close" onclick="closeReviewModal()" style="position: absolute; top: 10px; right: 15px; font-size: 24px; cursor: pointer; color: #666;">&times;</span>
                <h2 style="margin-top: 0; margin-bottom: 20px; color: #333;">评价订单</h2>
                <input type="hidden" id="review-order-id">
                <label for="review-rating" style="display: block; margin-bottom: 5px; font-weight: 500;">评分:</label>
                <select id="review-rating" required style="width: 100%; padding: 8px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 4px;">
                ...
                <label for="review-content" style="display: block; margin-bottom: 5px; font-weight: 500;">评价内容:</label>
                <textarea id="review-content" rows="4" placeholder="请输入评价内容..." required style="width: 100%; padding: 8px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 4px; resize: none;"></textarea>

                <button type="button" class="submit-btn" onclick="submitReview()" style="background: #b3001b; color: white; border: none; padding: 8px 20px; border-radius: 4px; cursor: pointer; margin-right: 10px;">提交评价</button>
                <button type="button" class="cancel-btn" onclick="closeReviewModal()" style="background: #f0f0f0; border: none; padding: 8px 20px; border-radius: 4px; cursor: pointer;">取消</button>
            </div>
        </div>
    `;
...
function loadOrderReviews() {
    initReviewModal();
    const contentArea = document.getElementById('my-content-area');
    contentArea.innerHTML = '<div class="loading">加载中...</div>';
    fetch('http://127.0.0.1:8000/api/user/received-orders', {
        headers: {
            'Authorization': `bearer ${localStorage.getItem('token')}`
        }
    })
    ...
    .then(data => {
        if (data.success) {
            renderReviewsPage(data.orders);
        } else {
            contentArea.innerHTML = `<div class="empty-state">${data.message || '获取订单失败'}</div>`;
        }
    })
    .catch(error => {
        contentArea.innerHTML = '<div class="empty-state">网络错误，无法加载订单</div>';
        console.error('加载评价订单失败:', error);
    });
}

// 渲染评价页面
function renderReviewsPage(orders) {
    const contentArea = document.getElementById('my-content-area');

    if (orders.length === 0) {
        contentArea.innerHTML = `
            <div class="empty-state">
                <img src="/images/empty_reviews.png" alt="暂无订单" class="empty-img">
                <p>暂无已收货的订单</p>
            </div>
        `;
        return;
    ...
    let html = `
        <div class="reviews-header">
            <h2 class="section-title">订单评价管理</h2>
            <p class="section-desc">共 ${orders.length} 个已收货订单</p>
        </div>
    ...
    orders.forEach(order => {
        html += `
            <div class="review-item" data-order-id="${order.order_id}">
                <div class="review-item-left">
                    <div class="order-id">订单编号: ${order.order_id}</div>
                    <div class="order-product">${order.goods}</div>
                    <div class="order-date">收货时间: ${order.time}</div>
                    <div class="review-status">
                        ${order.is_reviewed ?
                            '<span class="status-tag reviewed">已评价</span>' :
                            '<span class="status-tag pending">待评价</span>'}
                    </div>
                </div>
                <div class="review-item-right">
                    <button class="review-btn ${order.is_reviewed ? 'view-review' : 'write-review'}"
                            onclick="${order.is_reviewed ?
                                `viewReview(${order.order_id})` :
                                `openReviewModal(${order.order_id})`}">
                        ${order.is_reviewed ? '查看评价' : '去评价'}
                    </button>
                </div>
            </div>
        `;
...
function submitReview() {
    ...
    fetch(`http://127.0.0.1:8000/api/orders/${orderId}/review`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `bearer ${localStorage.getItem('token')}`
...
function showReviewDetailModal(orderId, review) {
    let starsHtml = '';
    const rating = parseInt(review.rating) || 0;
    for (let i = 1; i <= 5; i++) {
        starsHtml += `<span style="color: ${i <= rating ? '#ffcc00' : '#ddd'}; font-size: 28px;">★</span>`;
    }

    const modalHtml = `
        <div id="review-detail-modal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 9999;">
            <div style="width: 400px; padding: 20px; background: white; border-radius: 8px;">
                <h3>订单 #${orderId} 评价详情</h3>
                ...
                <div style="margin: 15px 0;">
                    ...
                    <p style="margin: 5px 0;">${review.comment || '无'}</p>
                </div>
                <div style="color: #999; margin: 10px 0;">
                    评价时间：${review.review_date || '未知'}
                </div>
                <button onclick="document.getElementById('review-detail-modal').remove()" style="width: 100%; padding: 8px; background: #b3001b; color: white; border: none; border-radius: 4px; cursor: pointer;">
                    关闭
                </button>
            </div>
        </div>
...
function updateOrderReviewStatus(orderId) {
    const reviewItem = document.querySelector(`.review-item[data-order-id="${orderId}"]`);
    if (!reviewItem) return;
    const reviewBtn = reviewItem.querySelector('.review-btn');
    reviewBtn.textContent = "查看评价";
    reviewBtn.className = "review-btn view-review";
    reviewBtn.onclick = function() {
        viewReview(orderId);
    };
    const statusTag = reviewItem.querySelector('.status-tag');
    statusTag.textContent = "已评价";
    statusTag.className = "status-tag reviewed";
}

function viewReview(orderId) {
    const oldModal = document.getElementById('review-detail-modal');
    ...
    .then(data => {
        if (data.success) {
            const review = {
                ...
                review_date: data.review.review_date
            };
            showReviewDetailModal(orderId, review);
        } else {
            alert('该订单暂无评价');
        }
    })
...
function showReviewDetailModal(orderId, review) {
    ...
    const modalHtml = `
        <div class="modal" id="review-detail-modal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 9999;">
            <div class="card" style="width: 400px; padding: 20px; background: white; border-radius: 8px; position: relative; z-index: 10000;">
                <span class="close" onclick="closeReviewDetailModal()" style="position: absolute; top: 10px; right: 15px; font-size: 24px; cursor: pointer;">&times;</span>
                <h2 style="margin-top: 0;">订单 #${orderId} 评价详情</h2>
                <div style="margin: 15px 0;">
                    <label style="display: block; font-weight: bold;">评分:</label>
                    <div class="stars" style="font-size: 28px; color: #ddd;">${starsHtml}</div>
                </div>
                <div style="margin: 15px 0;">
                    <label style="display: block; font-weight: bold;">评价内容:</label>
                    <p style="margin: 5px 0;">${review.comment}</p>
                </div>
                <div style="margin: 15px 0;">
                    <label style="display: block; font-weight: bold;">评价时间:</label>
                    <p style="margin: 5px 0;">${review.review_date}</p>
                </div>
                <button class="cancel-btn" onclick="closeReviewDetailModal()" style="padding: 8px 20px; background: #f0f0f0; border: none; border-radius: 4px; cursor: pointer;">关闭</button>
            </div>
        </div>
    `;
    document.body.insertAdjacentHTML('beforeend', modalHtml);
